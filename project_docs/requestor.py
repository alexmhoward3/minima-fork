...
def normalize_windows_path(container_path, local_base_path):
    """
    Normalize a container path to a Windows path while preserving proper structure.
    """
    logger.debug(f"normalize_windows_path input: container_path={container_path}, local_base_path={local_base_path}")
    
    # First, get the relative path from the container path
    if container_path.startswith('/usr/src/app/local_files/'):
        relative_path = container_path[len('/usr/src/app/local_files/'):]
    else:
        relative_path = container_path
    
    # Remove any leading/trailing slashes
    relative_path = relative_path.strip('/')
    logger.debug(f"Relative path after stripping container prefix: {relative_path}")
    
    # Split into directory components
    path_parts = [p for p in relative_path.split('/') if p]
    logger.debug(f"Path parts: {path_parts}")
    
    # Get the base directory (without 'Obsidian Vault')
    base_dir = os.path.dirname(local_base_path)
    logger.debug(f"Base directory: {base_dir}")
    
    # Build final path
    final_path = os.path.join(base_dir, 'Obsidian Vault', *path_parts)
...
async def request_deep_search(query):
    ...
    local_files_path = os.environ.get('LOCAL_FILES_PATH', 'C:\\Users\\Alex\\OneDrive\\Apps\\remotely-save\\Obsidian Vault')
    
    # Ensure the path is normalized
    local_files_path = os.path.normpath(local_files_path)
    logger.debug(f"Using local_files_path: {local_files_path}")
    
    if not os.path.exists(local_files_path):
        logger.warning(f"LOCAL_FILES_PATH does not exist: {local_files_path}")
                ...
                if query.include_raw:
                    ...
                    for result in data.get("raw_results", []):
                        ...
                        if file_path != 'Unknown':
                            # Use the normalize_windows_path function
                            file_path = normalize_windows_path(file_path, local_files_path)
